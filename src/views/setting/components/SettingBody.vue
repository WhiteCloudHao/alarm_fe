<template>
    <div class="setting-body">
        <div class="avatar flex mt-7">
         <img :src="avartarImage" alt="Avatar" class="rounded-full w-20 ">
         <p class="flex items-center ml-4">
           <span>{{ name }}</span>
         </p>
       </div>
       <div class="bg-white mt-8 pb-1 pt-1">
         <div class="flex mb-1 p-1" @click="redirectAppearence">
            <div class="flex items-center">
                <svgicon name="icon_setting_appearance" class="" width="24" height="24" color="#292D32" />
            </div>
            <span class="mx-4 text-black text-lg">Appearance</span>
            <div class="flex items-center ml-auto">
                <svgicon name="icon_next" width="14" height="14" color="#292D32" class="" />
            </div>
         </div>
         <div class="flex  my-1 p-1"  @click="redirectSound">
            <div class="flex items-center">
                <svgicon name="icon_setting_sound" width="24" height="24" color="#292D32" class="" />
            </div>
            <span class="mx-4 text-black text-lg">Sound & Notification</span>
            <div class="flex items-center ml-auto">
                <svgicon name="icon_next" width="14" height="14" color="#292D32" class="" />
            </div>
         </div>
         <div class="flex my-1 p-1" @click="redirectDate">
            <div class="flex items-center">
                <svgicon name="icon_setting_date" width="24" height="24" color="#292D32" class="" />
            </div>
            <span class="mx-4 text-black text-lg">Date & Time</span>
            <div class="flex items-center ml-auto">
                <svgicon name="icon_next" width="14" height="14" color="#292D32" class="" />
            </div>
         </div>
         <div class="flex  my-1 p-1" @click="redirectGeneral">
            <div class="flex items-center">
                <svgicon name="icon_setting_general" width="24" height="24" color="#292D32" class="" />
            </div>
            <span class="mx-4 text-black text-lg">General</span>
            <div class="flex items-center ml-auto">
                <svgicon name="icon_next" width="14" height="14" color="#292D32" class="" />
            </div>
         </div>
       </div>
       <div class="bg-white mt-4 pb-1 pt-1">
         <div class="flex mb-1 p-1" @click="redirectHelp">
            <div class="flex items-center">
                <svgicon name="icon_setting_help" class="" width="24" height="24" color="#292D32" />
            </div>
            <span class="mx-4 text-black text-lg">Help & Feedback</span>
            <div class="flex items-center ml-auto">
                <svgicon name="icon_next" width="14" height="14" color="#292D32" class="" />
            </div>
         </div>
         <div class="flex  my-1 p-1" @click="redirectFollow">
            <div class="flex items-center">
                <svgicon name="icon_setting_follow_us" width="24" height="24" color="#292D32" class="" />
            </div>
            <span class="mx-4 text-black text-lg">Follow Us</span>
            <div class="flex items-center ml-auto">
                <svgicon name="icon_next" width="14" height="14" color="#292D32" class="" />
            </div>
         </div>
         <div class="flex  my-1 p-1" @click="redirectAbout">
            <div class="flex items-center">
                <svgicon name="icon_setting_about" width="24" height="24" color="#292D32" class="" />
            </div>
            <span class="mx-4 text-black text-lg">About</span>
            <div class="flex items-center ml-auto">
                <svgicon name="icon_next" width="14" height="14" color="#292D32" class="" />
            </div>
         </div>
       </div>
       <div class="bg-white mt-6 py-2 text-red-600 text-lg">
         <button @click="signOut">Sign Out</button>
       </div>

    </div>
</template>

<script>
import '@/icons/icon_setting_appearance';
import '@/icons/icon_setting_sound';
import '@/icons/icon_setting_date';
import '@/icons/icon_setting_general';
import '@/icons/icon_next';
import '@/icons/icon_setting_help';
import '@/icons/icon_setting_follow_us';
import '@/icons/icon_setting_about';
export default {
   name: 'SettingBody',
   data() {
        return {
            avartarImage: 'data:image/jpeg;base64,',
            name: 'Nguyễn Xuân Nam'
        
        }
   },
   methods: {
    redirectAppearence() {
        this.$router.push('/setting/appearance')
    },
    redirectSound() {
        this.$router.push('/setting/sound-and-notification')
    },
    redirectDate() {
        this.$router.push('/setting/date-and-time')
    },
    redirectGeneral() {
        this.$router.push('/setting/general')
    },
    redirectHelp() {
        this.$router.push('/setting/help-and-feedback')
    },
    redirectFollow() {
        this.$router.push('/setting/follow-us')
    },
    redirectAbout() {
        this.$router.push('/setting/about')
    },
    signOut() {
        // 
    },
   }
}
</script>

<style>

</style>